...
<div style="float: left;">
    ...
    <table align="center">
        <tr>
            <th>День | Группа</th>
            <th colspan="2">Четная неделя</th>
            <th>Нечетная неделя</th>
        </tr>
<?php
       for($i=0; $i!=5; $i++) { ?>
                ...
                <tr>
                    ...
                    <td><?php 
                        // Выборка предметов в определенный день в определенной группе в четную неделю
                            $tt_query="SELECT DISTINCT lecturers.lect_name, subjects.subj_name, timetable.tt_day , timetable.tt_num, timetable.tt_week
                            FROM subjects, timetable, specialities, lecturers
                            WHERE timetable.subject=subjects.subj_id AND subjects.lecturer=lecturers.lect_id AND (tt_week='Четная' or tt_week='Всегда') 
                            AND lecturers.lect_name='$lect_name' AND tt_day='$days[$i]' AND timetable.tt_num='$j';";                                                
                    ...
                    <td><?php 
                            ...
                            $tt_uneven_query="SELECT DISTINCT lecturers.lect_name, subjects.subj_name, timetable.tt_day , timetable.tt_num, timetable.tt_week
                            FROM subjects, timetable, specialities, lecturers
                            WHERE timetable.subject=subjects.subj_id AND subjects.lecturer=lecturers.lect_id AND (tt_week='Нечетная' or tt_week='Всегда') 
                            AND lecturers.lect_name='$lect_name' AND tt_day='$days[$i]' AND timetable.tt_num='$j';";                                                
